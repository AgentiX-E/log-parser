...
describe('LogHub-2k Benchmark', () => {
  it('should parse SSH logs with GA >= 0.5', () => {
    ...
    // First pass: get actual template IDs from the pipeline
    const templateIdMap = new Map<string, string>();
    const parsed: ParsedLogEntry[] = SSH_LOGS.map((log, i) => {
      const result = pipeline.parse(log);
      // Map the first 2 logs to one group, last 2 to another
      const group = i < 2 ? 'group-a' : 'group-b';
      if (!templateIdMap.has(group)) {
        templateIdMap.set(group, String(result.templateId));
      }
      return { logId: String(i), template: result.template, eventId: String(result.templateId) };
    });

    // Build ground truth using the actual pipeline template IDs
    const groundTruth: GroundTruthEntry[] = SSH_LOGS.map((_, i) => ({
      logId: String(i),
      template: 'benchmark-ground-truth',
      eventId: templateIdMap.get(i < 2 ? 'group-a' : 'group-b') ?? 'unknown',
    }));

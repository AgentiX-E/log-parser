...
function mockObj(overrides: Record<string, unknown> = {}) {
  return {
    object: { template: 't', variables: [], confidence: 1 },
...
describe('OpenAICompatibleProvider', () => {
  ...
  it('ollama modelId', () => {
    expect(new OpenAICompatibleProvider({ provider: 'ollama', model: 'qwen2.5:7b' }).modelId).toBe(
      'ollama/qwen2.5:7b',
    );
  });
  it('openai modelId', () => {
    expect(
      new OpenAICompatibleProvider({ provider: 'openai', model: 'gpt-4o', apiKey: 'sk' }).modelId,
    ).toBe('openai/gpt-4o');
  });
  it('deepseek modelId', () => {
    expect(
      new OpenAICompatibleProvider({ provider: 'deepseek', model: 'dc', apiKey: 'sk' }).modelId,
    ).toBe('deepseek/dc');
  });
  it('anthropic modelId', () => {
    expect(
      new OpenAICompatibleProvider({ provider: 'anthropic', model: 'c3h', apiKey: 'sk' }).modelId,
    ).toBe('anthropic/c3h');
  });
  it('custom baseURL', () => {
    expect(
      new OpenAICompatibleProvider({ provider: 'custom', model: 'm', baseURL: 'https://x.com/v1' })
        .modelId,
    ).toBe('custom/m');
  });
  it('throws on invalid provider', () => {
    expect(() => new OpenAICompatibleProvider({ provider: 'bad' as never, model: 't' })).toThrow(
      /invalid provider type/,
    );
  });
  it('throws on empty model', () => {
    expect(() => new OpenAICompatibleProvider({ provider: 'ollama', model: '' })).toThrow(
      /model is required/,
    );
  });
  it('special chars in model', () => {
    expect(
      new OpenAICompatibleProvider({ provider: 'ollama', model: 'qw:7b-q4_K_M' }).modelId,
    ).toBe('ollama/qw:7b-q4_K_M');
  });

  // Happy path
  it('extracts template via structured output', async () => {
    gObj.mockResolvedValueOnce(
      mockObj({
        object: {
          template: 'User <*> logged in from <IP>',
          variables: [{ position: 1, value: 'a', category: 'GENERIC' }],
          confidence: 0.95,
        },
  ...
  it('empty samples returns zeros', async () => {
    const r = await new OpenAICompatibleProvider({
      provider: 'ollama',
      model: 'q',
    }).extractTemplate([]);
  ...
  it('falls back to generateText on error', async () => {
    ...
    gTxt.mockResolvedValueOnce({
      text: '{"template":"T","variables":[],"confidence":0.9}',
      usage: { promptTokens: 10, completionTokens: 5 },
      finishReason: 'stop',
      warnings: undefined,
      rawResponse: { headers: {} },
  ...
  it('fallback with non-array variables', async () => {
    gObj.mockRejectedValueOnce(new Error('fail'));
    gTxt.mockResolvedValueOnce({
      text: '{"template":"t","variables":"x","confidence":0.8}',
      usage: undefined,
      finishReason: 'stop',
      warnings: undefined,
      rawResponse: { headers: {} },
  ...
  it('ollama default baseURL branch', () => {
    expect(new OpenAICompatibleProvider({ provider: 'ollama', model: 'llama3' }).modelId).toBe(
      'ollama/llama3',
    );
  });
  it('openai default baseURL branch', () => {
    expect(
      new OpenAICompatibleProvider({ provider: 'openai', model: 'gpt-4', apiKey: 'sk' }).modelId,
    ).toBe('openai/gpt-4');
  });
  ...
  it('anthropic default baseURL branch', () => {
    expect(
      new OpenAICompatibleProvider({ provider: 'anthropic', model: 'c3h', apiKey: 'sk-ant' })
        .modelId,
    ).toBe('anthropic/c3h');
  });
  it('custom default baseURL branch', () => {
    expect(new OpenAICompatibleProvider({ provider: 'custom', model: 'm' }).modelId).toBe(
      'custom/m',
    );
  });
  ...
  it('fallback: usage has no promptTokens defaults to 0', async () => {
    gObj.mockRejectedValueOnce(new Error('fail'));
    gTxt.mockResolvedValueOnce({
      text: '{"template":"t","variables":[],"confidence":0.8}',
  ...
  it('happy path: usage empty object defaults both tokens to 0', async () => {
    gObj.mockResolvedValueOnce(
      mockObj({ object: { template: 't', variables: [], confidence: 1 }, usage: {} }) as never,
    );
    const r = await new OpenAICompatibleProvider({
      provider: 'ollama',
      model: 'q',
